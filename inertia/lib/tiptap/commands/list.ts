...
export const commandList = [
  ...
  },
  {
    ...
    command: ({ editor, range }: CommandParams) => {
      let callList = editor.chain().focus()
      if (range) return callList.deleteRange(range).setBlockquote().run()
  ...
  },
  {
    ...
    command: ({ editor, range }: CommandParams) => {
      let callList = editor.chain().focus()
      if (range) return callList.deleteRange(range).setHorizontalRule().run()
      callList.setHorizontalRule().run()
    },
  },
  {
    name: 'ul',
